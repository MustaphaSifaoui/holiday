...
$search_hdat         = GETPOST('hdat', 'alpha');
...
$arrayfields = array(
	'cp.ref'=>array('label'=>$langs->trans("Reference"), 'checked'=>1),
...
$sql .= " cp.hdat as hdat,"; // new filed added
...
// Start hour (hdat)
if (!empty($search_hdat)) {
    $sql .= " AND cp.hdat = '".$db->escape($search_hdat)."'\n"; // Replace hda with hdat if needed
}
...
if ($resql) {
	...
	if ($id > 0) {		// For user tab
		...
		$newcardbutton = dolGetButtonTitle($langs->trans('MenuAddAT'), '', 'fa fa-plus-circle', DOL_URL_ROOT.'/holiday/autorization/card.php?action=create', '', $user->rights->holiday->write);
    ...
    if (!empty($arrayfields['cp.hdat']['checked'])) {
        print_liste_field_titre($arrayfields['cp.hdat']['label'], $_SERVER["PHP_SELF"], "cp.hdat", "", $param, '', $sortfield, $sortorder, 'center ');
    }
